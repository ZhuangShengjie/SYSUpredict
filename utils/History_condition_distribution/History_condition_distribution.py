...
def Month_Vars_distribution(Month_Pre_input, Month_Prs_input, Month_Temp_input, Month_Win_input, Month_RHU_input,
                            sub_basin_name):
    """函数用于将输入的面平均变量（月尺度）转换成旬尺度上的面平均"""
    """请务必保证输入的变量为数字"""
    current_dir = os.path.dirname(os.path.abspath(__file__))  # 构建访问数据的相对路径
    # 读取已经保存为npy文件的数据库
    file_name_1 = 'Month_Areamean_DataBase_in_' + sub_basin_name + '_1980_2020.npy'
    file_name_2 = 'Month_Areamean_Range_in_' + sub_basin_name + '_1980_2020.npy'
    file_name_3 = 'TenDayscale_Areamean_DataBase_in_' + sub_basin_name + '_1980_2020.npy'
    data_file_path1 = os.path.join(current_dir, file_name_1)
    data_file_path2 = os.path.join(current_dir, file_name_2)
    data_file_path3 = os.path.join(current_dir, file_name_3)
    Month_Areamean_DataBase = np.load(data_file_path1, allow_pickle=True)
    Month_Areamean_Range = np.load(data_file_path2, allow_pickle=True)
    TenDays_Areamean_DataBase = np.load(data_file_path3, allow_pickle=True)
    ...
    #elif Month_Pre_input > float(His_Pre_rate_range[1]) * 1.2 and Month_Pre_input != 9999:  # 检验输入数值是不是超越设定的最大阈值
        #print("输入的降水数据超越历史阈值")

    #elif (Month_Prs_input != 9999 and Month_Prs_input < float(His_Prs_range[0]) * 0.8) or (
            #Month_Prs_input != 9999 and Month_Prs_input > float(His_Prs_range[1]) * 1.2):
        #print("输入的压强数据超越历史阈值")

    #elif (Month_Temp_input != 9999 and Month_Temp_input < float(His_Temp_range[0]) * 0.8) or (
            #Month_Temp_input != 9999 and Month_Temp_input > float(His_Temp_range[1]) * 1.2):
        #print("输入的气温数据超越历史阈值")

    #elif (Month_Win_input != 9999 and Month_Win_input < float(His_Win_range[0]) * 0.8) or (
            #Month_Win_input != 9999 and Month_Win_input > float(His_Win_range[1]) * 1.2):
        #print("输入的风速数据超越历史阈值")

    #elif (Month_RHU_input != 9999 and Month_RHU_input < float(His_RHU_range[0]) * 0.8) or (
            #Month_RHU_input != 9999 and Month_RHU_input > float(His_RHU_range[1]) * 1.2):
            ...
            for m in range(1, 4, 1):  # 对旬进行循环
                for i in range(rows):  # 按照相似度的索引进行
                    if Var_data_2[pos_2D[i, 0], pos_2D[i, 1]] == 0:
                        TenDays_Ratio_1[i, m - 1] = 0  # 存储计算出来的3旬结果
                    else:
                        if m == 1 or m == 2:  # 上中旬
                            b = (Var_data[pos_2D[i, 0], (pos_2D[i, 1]) * 3 + m - 1] / (
                                        Var_data_2[pos_2D[i, 0], pos_2D[i, 1]] * 3)) * Weight_index[i]
                        else:
                            b = (Var_data[pos_2D[i, 0], (pos_2D[i, 1]) * 3 + m - 1] / (
                                        Var_data_2[pos_2D[i, 0], pos_2D[i, 1]] * 3)) * Weight_index[i]  # 计算该旬在月数据下的比例
...
if __name__ == "__main__":
    ...
    Result = Month_Vars_distribution(Month_Pre_input, Month_Prs_input,
